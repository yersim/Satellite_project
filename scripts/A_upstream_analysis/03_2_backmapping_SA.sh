...
#SBATCH --job-name backmapping_saliva
...
## Array variables
config=/users/syersin2/Satellite/scripts/A_upstream_analysis/config_saliva.txt
cd ${index_indir}
...
for reads in $(awk 'NR >= 2 && NR <= 40 {print $2}' $config)
    do
