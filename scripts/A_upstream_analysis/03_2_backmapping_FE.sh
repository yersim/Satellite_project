...
#SBATCH --job-name backmapping_feces
...
## Array variables
config=/users/syersin2/Satellite/scripts/A_upstream_analysis/config_feces.txt
cd ${index_indir}
...
for reads in $(awk 'NR >= 2 && NR <= 45 {print $2}' $config)
    do
