...
setwd("/Users/admin/Documents/VLab_projects/01_Projects_MAIN/Satellite/Analysis/16S_data_analysis")
...
# Forward reads length: 250 bp
# quality decrease only at the last few bp
# # Drop at the beginning for the first 5 bp
plotQualityProfile(fnRs[1:4])
# Reverse reads length: 250 bp
# Quality decrease around 180-200 bp
# Drop at the beginning for the first 5 bp
```
...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, 
                     truncLen=c(245,200), trimLeft = c(5,5),
                     maxN=0, maxEE=c(6,8), truncQ=c(2,2),
                     rm.phix=TRUE, verbose=TRUE, matchIDs=TRUE,
                     compress=TRUE, multithread=TRUE) 
# We increase maxEE (reads with higher than maxEE "expected errors" will be discarded after truncation) from 2 to 6 and 8 to keep around 85% of our reads
# Using maxEE =2, around 75% of the reads were retained
...
write.table(track, "Tables/read_retention_table_Satellite.txt", quote=F, row.names=T, col.names=T, sep="\t")
...
# Save taxonomy table
write.table(taxa, "Tables/taxonomy_table_Satellite.txt", sep="\t", quote=F, row.names=T, col.names=T)
...
# Save sequence table
write.table(otu, "Tables/abundance_table_Satellite.txt", sep="\t", quote=F, row.names=T, col.names=T)
