...
md <- md %>%
  mutate(Exudation_AAT = "")
md$Exudation_AAT[which(md$`AAT  (mg/g)` > 0.15)] <- "Exudation"
md$Exudation_AAT[which(md$`AAT  (mg/g)` <= 0.15)] <- "No_exudation"
...
# gt summary table 
```{r}
md_sum <- md %>% select(id, Stunting_status, nagem, sexe, voie_accou, Exudation_AAT, Gut_inflammation_Calpro,
...
md_sum %>% 
  select(Stunting_status, nagem, Age_group, sexe, voie_accou,Exudation_AAT,
...
seq_metag$SampleName[110:129] <- str_c(seq_metag$SampleName[110:129], "FE")
...
# Table for supplementary file
md_supp <- md_sat %>% select(c(id_modif, id, pays, sexe, nagem, ageunit, voie_accou, body_site, nheight, nweight, haz_cont, waz_cont,
                               whz_cont, bmi_cont, whz, haz, stunted, stuntedandwasted, stuntedorwasted, Stunting_level, Stunting_status,
                               `AAT  (mg/g)` ,Exudation_AAT,
                              `CALPROTECTINE (μg/g)`,Gut_inflammation_Calpro, crp,Systemic_inflammation, T, Leaky_gut,
                             SeqBatch, Total, Retained, Removed , `Retained%`))

md_supp <- md_supp %>%
  filter(!is.na(Total))

write_xlsx(md_supp, "/Users/admin/Documents/VLab_projects/01_Projects_MAIN/Satellite/Analysis/Metadata/Metadata_satellite_suppTab1.xlsx")

# Data for adding to Pastobiome project:
md_pastobiome_SAT <- md_sat %>%
  filter(body_site == "Feces")
md_pastobiome_SAT <-  md_pastobiome_SAT %>% select(id_modif,id, nagem, sexe, pays, alim,haz_cont, whz_cont, Stunting_level, Stunting_status, eau_courante, eau_puit, eau_minerale, eau_aut_p, eau_traitee, eau_chlo, eau_filtre, eau_bouillie, parasite, giardiase, entamoeba, parasite_aut, nom_parasite1)
write_xlsx(md_pastobiome_SAT, "/Users/admin/Documents/VLab_projects/01_Projects_MAIN/Satellite/Analysis/Metadata/Metadata_satellite_PastobiomeProject.xlsx")
```
...
md_oral <- md_sat %>%
  ...
  filter(!is.na(SeqBatch))


ena_saliva <- read.delim2("/Users/admin/Documents/VLab_projects/01_Projects_MAIN/Satellite/Manuscript/ENA/Checklist_GSC-MIxS human oral_ERC000016_1762346711786.tsv", skip = 1)
colnames(ena_saliva)
ena_saliva <- ena_saliva[1:39,]
ena_saliva$tax_id <- "9606"
ena_saliva$scientific_name <- "Homo sapiens"
ena_saliva$sample_alias <- md_oral$id_modif
ena_saliva$sample_title <- "Saliva sample"

ena_saliva$collection.date <- md_oral$dt_inc
ena_saliva$geographic.location..latitude. <- "7.03236"
ena_saliva$geographic.location..longitude. <- "19.998123"

write_xlsx(ena_saliva, "/Users/admin/Documents/VLab_projects/01_Projects_MAIN/Satellite/Manuscript/ENA/ENA_saliva_sample_list.xlsx")
...
ena_gut <- read.delim2("/Users/admin/Documents/VLab_projects/01_Projects_MAIN/Satellite/Manuscript/ENA/Checklist_GSC-MIxS human gut_ERC000015_1762346857243.tsv", skip = 1)
ena_gut <- ena_gut[1:90,]
ena_gut$sample_alias <- md_gut$id_modif
ena_gut <- ena_gut %>%
  mutate(sample_title = case_when(
    str_detect(sample_alias,"FE") ~ "human fecal microbiota",
    str_detect(sample_alias,"GA") ~ "human gastric aspirate microbiota",
    str_detect(sample_alias,"DU") ~ "human duodenal aspirate microbiota"
  )) %>%
  mutate(sample_description = case_when(
    str_detect(sample_alias,"FE") ~ "children fecal microbiota, sequenced for metagenomic analysis",
    str_detect(sample_alias,"GA") ~ "children gastric aspirate microbiota, sequenced for metagenomic analysis",
    str_detect(sample_alias,"DU") ~ "children duodenal aspirate microbiota, sequenced for metagenomic analysis"
  ))  %>%
  mutate(environmental.medium = case_when(
    str_detect(sample_alias,"FE") ~ "fecal material",
    str_detect(sample_alias,"GA") ~ "gastric material",
    str_detect(sample_alias,"DU") ~ "duodenal material"
  ))  %>%
  mutate(local.environmental.context = "digestive tract environment")

ena_gut$collection.date <- md_gut$dt_inc

write_xlsx(ena_gut, "/Users/admin/Documents/VLab_projects/01_Projects_MAIN/Satellite/Manuscript/ENA/ENA_gut_sample_list.xlsx")
```
...
md_sat_fe <- dplyr::select(md_sat_fe, c("id_modif", "stunted", "sexe", "nagem", "nweight", "statut_nut", "Gut_inflammation_Calpro",
                                        "SeqBatch",
                                 "Exudation_AAT", "Systemic_inflammation", "Leaky_gut", "Total", "Retained", "Removed",
...
#AAT
ggplot(md_fe_sub, aes(x = Removed_per, y = as.numeric(`AAT  (mg/g)`), color = Exudation_AAT)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
...
summary(lm(Removed_per ~ Exudation_AAT + nagem + stunted + voie_accou + atb_suivi, data = md_fe_sub))
...
summary(lm(Removed_per ~ Exudation_AAT + nagem + stunted, data = md_fe_noout))
...
Duodenal biomass and haz
```{r}
biomass_duo <- read_excel("/Users/admin/Documents/VLab_projects/01_Projects_MAIN/Satellite/Samples_Sequencing/qPCR/Duodenum_results.xlsx", sheet = 1)

biomass_duo <- biomass_duo %>% filter(!is.na(`log10(CFU/mL)`))

md_d <- merge(md_d, biomass_duo, by.x = "id_modif", by.y = "SampleID")


cor.test(md_d$haz_cont,md_d$`With dilution factor`, method = "spearman")
cor.test(md_d$haz_cont,md_d$`With dilution factor`, method = "pearson")
